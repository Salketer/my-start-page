Widgets = {
    ...
    register: function (name, widget) {
        ...
        widget.name = name;
        this.available_widgets.push(widget);
    ...
    WidgetBase: function WidgetBase() {
        return {
            width_min: 1,
            width_max: 0,
            height_min: 1,
            ...
            modify: function (change) {
                widget = this;
                change = this.beforeModify(change);
                Widgets_Collection.update(this._id, {
                    $set: change
                }, {}, function (error, affected_docs) {
                    if (!error && affected_docs > 0) {
                        widget.afterModify(change);
                    }
                });
            },
    ...
    instanciate: function (widget_document) {
        if (this.widgetsList.indexOf(widget_document.type) >= 0) {
            return _.deepExtend({}, this.available_widgets[this.widgetsList.indexOf(widget_document.type)],
                widget_document);
        } else {
